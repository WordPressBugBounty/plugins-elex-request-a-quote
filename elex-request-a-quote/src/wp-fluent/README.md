...
## Example
```PHP
// You can use the global WPFluentELEXRAQ() function
$user = WPFluentELEXRAQ()->table('users')->find(1);
...
// Or, simply use the global WPFluentELEXRAQ() function.
// It handles all the ncessary initial setup.
WPFluentELEXRAQ()->table('my_table')->where('name', '=', 'admin');
```
